...
const arePropsEqual = (prev: ChartRendererProps, next: ChartRendererProps): boolean => {
  const p = prev.component;
  const n = next.component;
  return (
    p.id === n.id &&
    p.chartType === n.chartType &&
    p.dataSetId === n.dataSetId &&
    p.xField === n.xField &&
    p.yField === n.yField &&
    p.config === n.config &&
    p.title === n.title &&
...
const ChartRenderer: React.FC<ChartRendererProps> = ({ component }) => {
  ...
  }, [component.dataSetId, dataSets, component.xField, component.yField, chartType]);
  ...
  const baseOption = useMemo(() => {
    ...
    tooltip: {
      trigger: CHART_WITH_ITEM_TRIGGER.includes(chartType) ? 'item' : 'axis',
    },
    legend: {
      show: legendShow,
  ...
  }, [cfg.title, component.title, chartType, cfg.titleFont, cfg.legend, cfg.showLegend]);
    ...
    switch (chartType) {
      case 'line':
        return {
          ...
          xAxis: {
            ...
            show: xCfg.show ?? true,
            name: xCfg.showTitle ? xCfg.title : '',
            axisLabel: { fontSize: xCfg.fontSize || 12, color: xCfg.color || '#666' },
          },
          yAxis: {
            type: 'value',
            show: yCfg.show ?? true,
            name: yCfg.showTitle ? yCfg.title : '',
          ...
          series: [{
            name: component.yField || '数值',
            data: chartData.values,
            type: 'line',
            smooth: true,
            itemStyle: { color: colorPalette[0] },
            label: {
              show: dlCfg.show ?? false,
              position: dlCfg.position || 'top',
              fontSize: dlCfg.fontSize || 12,
              color: dlCfg.color || '#333',
            },
            areaStyle: {
              color: {
          ...
          series: [{
            ...
            itemStyle: {
              color: {
                ...
                colorStops: [
                  { offset: 0, color: colorPalette[1] || '#52c41a' },
      ...
      case 'pie':
        ...
        return {
          ...
          series: [{
            ...
            label: {
              show: dlCfg.show ?? true,
              formatter: '{b}: {d}%',
              fontSize: dlCfg.fontSize || 12,
              color: dlCfg.color || '#333',
              position: dlCfg.position === 'inside' ? 'inside' : 'outside',
            },
      ...
      case 'scatter':
        ...
        return {
          ...
          xAxis: {
            type: 'value',
            name: xCfg.showTitle ? xCfg.title : component.xField,
            show: xCfg.show ?? true,
            axisLabel: { fontSize: xCfg.fontSize || 12, color: xCfg.color || '#666' },
            splitLine: { show: gridShow, lineStyle: { color: gridColor, width: gridWidth, type: gridType } },
          },
          yAxis: {
            type: 'value',
            name: yCfg.showTitle ? yCfg.title : component.yField,
            show: yCfg.show ?? true,
            axisLabel: { fontSize: yCfg.fontSize || 12, color: yCfg.color || '#666' },
            splitLine: { show: gridShow, lineStyle: { color: gridColor, width: gridWidth, type: gridType } },
          },
          ...
          series: [{
            ...
            areaStyle: {
              color: {
                type: 'linear', x: 0, y: 0, x2: 0, y2: 1,
                colorStops: [
                  { offset: 0, color: 'rgba(47, 84, 235, 0.5)' },
                  { offset: 1, color: 'rgba(47, 84, 235, 0.05)' },
                ],
              },
            },
          }],
            ...
            data: [
              {
                ...
                itemStyle: { color: colorPalette[0] },
                label: { show: dlCfg.show ?? false, fontSize: dlCfg.fontSize || 12, color: dlCfg.color || '#333' },
              },
            ],
          }],
        };
      ...
      case 'gauge': {
        // 仪表盘：对绑定字段求和，自动计算上限
        const gaugeTotal = chartData.hasData
          ? chartData.values.reduce((a, b) => a + b, 0)
          : 0;
        const gaugeMax = chartData.hasData
          ? Math.ceil(Math.max(...chartData.values, 1) * 1.5)
          : 100;
        const gaugeName = component.yField || '指标';
        const gaugePercent = chartData.hasData
          ? Math.round((gaugeTotal / gaugeMax) * 100)
          : 0;
        ...
        return {
          ...
          series: [{
            ...
            anchor: {
              ...
              showAbove: true,
              size: 16,
              itemStyle: { borderWidth: 6, borderColor: colorPalette[0] },
            },
            title: {
              show: true,
              offsetCenter: [0, '28%'],
            ...
            detail: {
              ...
              formatter: (val: unknown) => {
                ...
                const pct = chartData.hasData
                  ? Math.round((v / gaugeMax) * 100)
                  : 0;
                return `{value|${v}}\n{percent|${pct}%}`;
              },
              rich: {
                value: { fontSize: 28, fontWeight: 'bold', color: '#333', padding: [0, 0, 4, 0] },
                percent: { fontSize: 14, color: '#999' },
              },
            },
      ...
      case 'heatmap': {
        ...
        for (let i = 0; i < heatCats.length; i++) {
          heatData.push([heatCats[i], heatCats[i], heatVals[i] || 0]);
        }
        ...
        return {
          ...
          visualMap: {
            ...
            left: 'center',
            bottom: 5,
            inRange: { color: ['#e6f7ff', '#91d5ff', colorPalette[0], '#0050b3'] },
          },
          series: [{
            name: component.yField || '数值',
            type: 'heatmap',
      ...
      case 'waterfall': {
        ...
        wfPlaceHolder.push(0);
        wfIncrease.push(wfValues[0] || 0);
        wfDecrease.push('-');
        for (let i = 1; i < wfValues.length - 1; i++) {
          if (wfValues[i] >= 0) {
            wfPlaceHolder.push(running);
            wfIncrease.push(wfValues[i]);
            wfDecrease.push('-');
            running += wfValues[i];
          } else {
            wfPlaceHolder.push(running + wfValues[i]);
            wfIncrease.push('-');
            wfDecrease.push(Math.abs(wfValues[i]));
            running += wfValues[i];
          }
        }
        wfPlaceHolder.push(0);
        wfIncrease.push(wfValues[wfValues.length - 1] || 0);
        wfDecrease.push('-');
        return {
          ...baseOption,
          xAxis: {
  ...
  }, [chartType, chartData, component, cfg, colorPalette, baseOption]);

  return (
    <ReactECharts
      key={`${component.id}-${component.chartType}-${component.dataSetId || 'none'}-${component.xField || 'none'}-${component.yField || 'none'}-r${dataSet?.rowCount ?? 0}-v${dataVersion}`}
      option={option}
...
export default React.memo(ChartRenderer, arePropsEqual);
